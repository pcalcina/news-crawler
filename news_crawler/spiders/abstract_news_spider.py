...
class AbstractNewsSpider(CrawlSpider):
    ...
    def __init__(self, domain, source_id, database, keywords, start_url):
        ...
        logging.info("Initializing crawling with domain=%s, keywords=%s, start_url=%s, database=%s" % 
                     (domain, self.keywords, str(self.start_urls), database))
            ...
            for article_link in self.get_article_links(response):                
                ...
                if url:                
                    if self.already_crawled(url):
                        logging.info( '[%d-2%d MERGING OLD] %s' % (self.next_page, i, url))
                        self.merge_keywords(url)
                    elif self.accept_new_articles:
                        logging.info( '[%d-2%d ADDING NEW] %s' % (self.next_page, i, url))
                        try:
                            yield scrapy.Request(str(url), 
                                             callback=self.parse_article)
                        except Exception as e:
                            logging.error('ERROR trying to crawling %s. Message=%s' % 
                                   (url, e))
                    else:
                        logging.info('[%d-2%d SKIPPING NEW] %s' % (self.next_page, i, url))
...
#         """Returns the url to begin the crawling."""
    ...
    def already_crawled(self, url):
        ...
        logging.debug("Verifying if %s URL is crawled, total = %d" % (url, total))
        return total > 0
    ...
    def has_more_pages(self):
        logging.info(' --- Page %s of %s --- ' % (self.next_page, self.final_page))
